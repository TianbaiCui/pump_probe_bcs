...
struct rhs_bcs_k_space {
    ...
    void operator() (const Doub x, VecDoub_I &y, VecDoub_O &dydx) {
        ...
        //double A = 0.;
        if ((x > 0.) && (x < tau)) {
            Ax = A0x*exp(-pow(x-tau/2,2)/(2.*sigma*sigma)) * cos(omega_pump*x);
            Ay = A0y*exp(-pow(x-tau/2,2)/(2.*sigma*sigma)) * cos(omega_pump*x);
            //Ax = A0x*exp(-pow(2*(x-tau)/tau,2)*log(2)) * cos(omega_pump*x);
            //Ay = A0y*exp(-pow(2*(x-tau)/tau,2)*log(2)) * cos(omega_pump*x);
...
int main(int argc, const char * argv[]) {
    ...
    for(int idxy = 0; idxy < L_kxy; idxy++){
        ...
        for (int idy = 0.; idy < L_kx; idy++){
            ...
            if (kxy-ky>=0){
               ...
               kstates[3*idxy*L_kx+3*idy+1] = -2*pi;
               kstates[3*idxy*L_kx+3*idy+2] = -2*pi;
            }
    ...
    const Doub h1 = 0.0001;
    const Doub hmin = 0.0;

    const Doub x1 = 0.0; // initial time
    const Doub x2 = tau; // final time
    ...
    Odeint<StepperDopr853<rhs_bcs_k_space>> ode(ystart, x1, x2, atol, rtol, h1, hmin, out, bcs_k);
    ode.integrate();

    cout << "ode.integrate finished in region 0 < t < tau. " << endl;
    ...
    for (int jdx = 0; jdx < N; jdx++) {
        ...
        if (xi >= -omega_D && xi <= omega_D && kstates[3*jdx+1] >= 0){
            DeltaRe_final += ps_ratio * out.ysave[3*jdx][out.count-1]*(V0/N);
            DeltaIm_final += ps_ratio * out.ysave[3*jdx+1][out.count-1]*(-V0/N);
            energy_final += (ps_ratio/N)*(2.*xi*out.ysave[3*jdx+2][out.count-1] + sqrt(xi*xi + Delta0*Delta0));
        }
        //energy += (1/(N*N)) * (2.*xi*y[3*idx*N + 3*idy + 2] + sqrt(xi*xi + Delta0*Delta0));
    }
     ...
     **************************************************************************/
    const Doub x3 = tau; // initial time
    const Doub x4 = x_max; // final time

    for (int idx = 0; idx < N; idx++) {
        ystart[3*idx] = out.ysave[3*idx][out.count-1];
        ystart[3*idx+1] = out.ysave[3*idx+1][out.count-1];
        ystart[3*idx+2] = out.ysave[3*idx+2][out.count-1];
    }
    Output out_2(2000);
    cout << "momentun space discretization used." << endl;
    ...
    Odeint<StepperDopr853<rhs_bcs_k_space2>> ode_2(ystart, x3, x4, atol, rtol, h1, hmin, out_2, bcs_k_2);
    ode_2.integrate();

    cout << " ode.integrate finished in region t > tau. " << endl;
    /********************************************
     **************** Output ********************
     ********************************************/
    ...
    sigma_string.erase(sigma_string.find_last_not_of('.') + 1, string::npos);
    tau_string.erase(tau_string.find_last_not_of('0') + 1, string::npos);
    tau_string.erase(tau_string.find_last_not_of('.') + 1, string::npos);
    ...
    if (output_delta == true) {
        outputFilename = "Output/Data(t)_Shimano-Ax=" + A0x_string  + "-Ay=" + A0y_string + "-omega_p=" + omega_pump_string + "-tau=" + tau_string + "-sigma=" + sigma_string + "-T1=" + T1_string + "-T2=" + T2_string + "-L_kxy=" + L_kxy_string + "-L_kx=" + L_kx_string + ".dat";
        outputFile.open(outputFilename);
        ...
        for (int idx = 0; idx < out.count; idx++) {
            outputFile << out.xsave[idx]*Delta0 << " ";
            ...
            double eta = 1.;
            //double A = 0.;
            if ((out.xsave[idx] > 0.) && (out.xsave[idx] < tau)) {
                //Ax = A0x * exp(-pow(2*(out.xsave[idx]-tau)/tau,2)*log(2)) * cos(omega_pump*out.xsave[idx]);
                //Ay = A0y * exp(-pow(2*(out.xsave[idx]-tau)/tau,2)*log(2)) * cos(omega_pump*out.xsave[idx]);
                Ax = A0x*exp(-pow(out.xsave[idx]-tau/2,2)/(2.*sigma*sigma)) * cos(omega_pump*out.xsave[idx]);
                Ay = A0y*exp(-pow(out.xsave[idx]-tau/2,2)/(2.*sigma*sigma)) * cos(omega_pump*out.xsave[idx]);
            ...
            for (int jdx = 0; jdx < N; jdx++) {
                ...
                if (kstates[3*jdx] >= -omega_D && kstates[3*jdx] <= omega_D && kstates[3*jdx+1]>=0){
                    DeltaRe_out += ps_ratio * out.ysave[3*jdx][idx]*(V0/N);
                    DeltaIm_out += ps_ratio * out.ysave[3*jdx+1][idx]*(-V0/N);
                    energy_out += (ps_ratio/N) * (2.*kstates[3*jdx]*out.ysave[3*jdx+2][idx] + sqrt(kstates[3*jdx]*kstates[3*jdx] + Delta0*Delta0));
                }
                //if (kstates[3*jdx] >= -omega_D && kstates[3*jdx] <= omega_D && kstates[3*jdx+1]>=0){
        ...
        for (int idx = 0; idx < out_2.count; idx++) {
            outputFile << out_2.xsave[idx]*Delta0 << " ";
            double DeltaRe_out = 0.;
            double DeltaIm_out = 0.;
            double energy_out = 0.;
            for (int jdx = 0; jdx < N; jdx++) {
                double xi = kstates[3*jdx];
                if (xi >= -omega_D && xi <= omega_D && kstates[3*jdx+1] >= 0){
                    DeltaRe_out += ps_ratio * out_2.ysave[3*jdx][idx]*(V0/N);
                    DeltaIm_out += ps_ratio * out_2.ysave[3*jdx+1][idx]*(-V0/N);
                    energy_out += ps_ratio/N * (2.*xi*out_2.ysave[3*jdx+2][idx] + sqrt(xi*xi + Delta0*Delta0));
                }
            }
            energy_out += -1./V0*(DeltaRe_out*DeltaRe_out + DeltaIm_out*DeltaIm_out + Delta0*Delta0);
            energy_out = energy_out/(Delta0*Delta0);
    ...
    if (output_A == true) {
            outputFilename = "Output/A(t)-A0_" + A0x_string + "-omega_p_" + omega_pump_string + "-tau_" + tau_string + "-sigma_" + sigma_string + ".dat";
            outputFile.open(outputFilename);
            ...
            for (int idx = 0; idx < out.count; idx++) {
                outputFile << out.xsave[idx]*Delta0 << " ";
                if ((out.xsave[idx] > 0.) && (out.xsave[idx] < tau)) {
                    outputFile << A0*exp(-pow(out.xsave[idx]-tau/2., 2)/(2.*sigma*sigma)) * cos(omega_pump*out.xsave[idx]);
                }
                else {
                    outputFile << 0.;
                }
    ...
    if (output_spin == true) {
        outputFilename = "Output/spinx_Ax=" + A0x_string  + "-Ay=" + A0y_string + "-omega_p=" + omega_pump_string + "-tau=" + tau_string + "-sigma=" + sigma_string + "-T1=" + T1_string + "-T2=" + T2_string + "-L_kxy=" + L_kxy_string + "-L_kx=" + L_kx_string + ".dat";
        outputFile.open(outputFilename);
        ...
        //double energy_A_out = 0.;
        for (int jdx = 0; jdx < N; jdx++) {
            if (kstates[3*jdx+1] >= 0){
              outputFile << kstates[3*jdx+1] << " " << kstates[3*jdx+2] << " " << out.ysave[3*jdx][out.count-1] << endl;
            }
        }
        outputFile.close();
    }
